...
func GetOffsetSpeedTarget(gunner Gunner, target *physical_model.PhysicalModel, mp *_map.Map, weaponSlot *body.WeaponSlot) (int, int) {
	...
	distPathUnit := timeToTarget * (target.GetCurrentSpeed() * (1000 / _const.ServerTick))
...
func GetBulletSpeedToTargetByWeaponSlot(xTarget, yTarget int, zTarget float64, mp *_map.Map, gunner Gunner, weaponSlot *body.WeaponSlot) float64 {
	...
	bulletSpeed := float64(weaponSlot.Weapon.BulletSpeed + weaponSlot.GetAmmo().BulletSpeed)
	startRadian := game_math.GetReachAngle(weaponPos.X, weaponPos.Y, xTarget, yTarget, zTarget, lvl, bulletSpeed, false, weaponSlot.GetAmmo().Type)
